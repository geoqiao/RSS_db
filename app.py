...
app = FastAPI(
    debug=False, title="My API", docs_url="/docs", openapi_url="/openapi.json"
)
...
distance_year = (
    datetime.datetime.strptime(f"{today.year + 1}-01-01", "%Y-%m-%d").date() - today
).days
...
distance_4_5 = (
    ...
    if distance_4_5 > 0
        ...
        datetime.datetime.strptime(f"{today.year + 1}-04-05", "%Y-%m-%d").date() - today
    ).days
)
...
distance_5_1 = (
    ...
    if distance_5_1 > 0
        ...
        datetime.datetime.strptime(f"{today.year + 1}-05-01", "%Y-%m-%d").date() - today
    ).days
)
...
distance_10_1 = (
    ...
    if distance_10_1 > 0
        ...
        datetime.datetime.strptime(f"{today.year + 1}-10-01", "%Y-%m-%d").date() - today
    ).days
)
